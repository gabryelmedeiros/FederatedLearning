...
def get_and_divide_dataset(num_clients, batch_size, max_dataset_size, k_folds=5, normalization_factor=255.0, seed=42):
    ...
    Splits the training data into k folds for cross-validation, where each fold
    acts as a validation set, and the remaining k-1 folds are used for client training data.

    Returns:
        client_train_datasets: A list of lists. client_train_datasets[i][k] is the tf.data.Dataset
                                for client i in fold k.
        validation_datasets: A list of tf.data.Dataset, validation_datasets[k] is the validation
                             dataset for fold k.
    """

    (train_images, train_labels), (test_images_orig, test_labels_orig) = tf.keras.datasets.mnist.load_data()

    if num_clients <= 0:
        raise ValueError("Number of clients must be greater than zero.")

    if max_dataset_size is not None and max_dataset_size > len(train_images):
        print(f"Warning: max_dataset_size {max_dataset_size} is bigger than the train dataset size ({len(train_images)}). Using full train dataset.")
        max_dataset_size = len(train_images)
    elif max_dataset_size is None:
         max_dataset_size = len(train_images) # Use full dataset if None

    # Use the specified subset of the training data
    train_images_subset = train_images[:max_dataset_size]
    train_labels_subset = train_labels[:max_dataset_size]

    # Preprocess (normalize and expand dimensions)
    train_images_subset = train_images_subset / normalization_factor
    test_images_orig = test_images_orig / normalization_factor # Normalize original test set too
    train_images_subset = np.expand_dims(train_images_subset, axis=-1)
    test_images_orig = np.expand_dims(test_images_orig, axis=-1)


    # Shuffle the training subset data
    np.random.seed(seed)
    indices = np.random.permutation(len(train_images_subset))
    train_images_subset = train_images_subset[indices]
    train_labels_subset = train_labels_subset[indices]

    # Calculate fold sizes for the training subset data
    total_subset_size = len(train_images_subset)
    fold_size = total_subset_size // k_folds
    if fold_size == 0:
        raise ValueError(f"Subset size ({total_subset_size}) is too small to create {k_folds} folds. Increase max_dataset_size or decrease k_folds.")

    # Check if the remaining training data (after taking out validation fold)
    # is sufficient for clients
    remaining_size_per_fold = total_subset_size - fold_size
    if remaining_size_per_fold < num_clients * batch_size:
         print(f"Warning: Remaining training data size per fold ({remaining_size_per_fold}) may be too small to provide each of {num_clients} clients with at least one batch of size {batch_size}. Consider increasing max_dataset_size or decreasing num_clients/batch_size.")
         # Or enforce: raise ValueError(...)

    # Create k folds for validation and client training data
    client_train_datasets = [[] for _ in range(num_clients)] # client_train_datasets[client_idx] is a list of datasets (one for each fold)
    validation_datasets = [] # validation_datasets[fold_idx] is the validation dataset for that fold

    for k in range(k_folds):
        print(f"Preparing data for Fold {k+1}/{k_folds}")
        # The k-th segment is the validation set for this fold
        val_images_fold = train_images_subset[k * fold_size:(k + 1) * fold_size]
        val_labels_fold = train_labels_subset[k * fold_size:(k + 1) * fold_size]
        validation_dataset_fold = tf.data.Dataset.from_tensor_slices((val_images_fold, val_labels_fold)).batch(batch_size)
        validation_datasets.append(validation_dataset_fold)

        # The remaining data is the training data for this fold
        train_images_fold = np.concatenate((train_images_subset[:k * fold_size], train_images_subset[(k + 1) * fold_size:]))
        train_labels_fold = np.concatenate((train_labels_subset[:k * fold_size], train_labels_subset[(k + 1) * fold_size:]))

        # Distribute the training data for this fold among clients
        images_per_client = len(train_images_fold) // num_clients
        if images_per_client == 0 and len(train_images_fold) > 0:
            # This means there's training data, but not enough for each client to get at least 1 sample
            # In a batch-based system, this might be problematic.
             print(f"Warning: Training data size for a fold ({len(train_images_fold)}) is too small to provide each of {num_clients} clients with at least 1 sample after validation split. Clients may receive empty datasets.")
        elif images_per_client == 0 and len(train_images_fold) == 0:
             print(f"Warning: Training data size for a fold ({len(train_images_fold)}) is zero. Clients will receive empty datasets.")


        for i in range(num_clients):
            client_start = i * images_per_client
            client_end = (i + 1) * images_per_client
            if client_start < len(train_images_fold): # Check if slice is valid
                 client_train_data = tf.data.Dataset.from_tensor_slices(
                     (train_images_fold[client_start:client_end],
                      train_labels_fold[client_start:client_end])
                 ).shuffle(buffer_size=min(len(train_images_fold), 1000)).batch(batch_size).cache().repeat()
            else:
                 # If client_start is out of bounds, the client gets an empty dataset
                 print(f"Warning: Client {i+1} in fold {k+1} would receive an empty dataset.")
                 client_train_data = tf.data.Dataset.from_tensor_slices((tf.constant([], dtype=train_images_fold.dtype), tf.constant([], dtype=train_labels_fold.dtype))).batch(batch_size).cache().repeat()

            client_train_datasets[i].append(client_train_data) # Append the dataset for this client in this fold

    # Return client training datasets for each fold and validation datasets for each fold
    return client_train_datasets, validation_datasets
